function generate_data_multistate(aseed)
    ...
    hazard0_12 = (2.0.*exp.(-2.0.*timeomega)) .* omega
    hazard0_13 = (2.0.*exp.(-2.0.*timeomega)) .* omega
    hazard0_24 = (2.0.*timeomega.*timeomega) .* omega
    hazard0_25 = (2.0.*timeomega.*timeomega) .* omega
    hazard0_35 = (2.0.*timeomega.*timeomega) .* omega
    hazard0_36 = (2.0.*timeomega.*timeomega) .* omega

    hazard0_true[1,2,:] = hazard0_12
    hazard0_true[1,3,:] = hazard0_13
    hazard0_true[2,4,:] = hazard0_24
    hazard0_true[2,5,:] = hazard0_25
    hazard0_true[3,5,:] = hazard0_35
    hazard0_true[3,6,:] = hazard0_36

    ndimz = 4
    betaz_true = fill(0.0, nstates, nstates, ndimz)
    betaz_true[1,2,:] = [0.5, 0.1, 0.1, 0.1]
    betaz_true[1,3,:] = [0.1, 0.5, 0.1, 0.1]
    betaz_true[2,4,:] = [0.1, 0.1, 0.4, 0.2]
    betaz_true[2,5,:] = [0.5, 0.1, 0.1, 0.1]
    betaz_true[3,5,:] = [0.1, 0.5, 0.1, 0.1]
    betaz_true[3,6,:] = [0.1, 0.1, 0.2, 0.4]
    ...
    for ii in 1:nind
        ...
        for tt in 1:ntimepoints
            ...
            sum(zzi[ii,:] .* betaz_true[current,5,:]),
            sum(zzi[ii,:] .* betaz_true[current,6,:])])
